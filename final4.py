# final_controller_v7.py
# This version detects both left and right hands, and adds POINT_LEFT/RIGHT gestures.
...
# --- SETUP ---
print("Starting AI Ghost Hand Controller V7 (Full Left/Right Control)...")
...
hands = mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.7,
                       min_tracking_confidence=0.7)  # Detect up to 2 hands
...
while cap.isOpened():
    ...
    if results.multi_hand_landmarks:
        for hand_idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            try:
                ...
                # POINT_LEFT / POINT_RIGHT: Index finger up, others down, and check x-position relative to wrist
                is_pointing = (index_tip.y < index_pip.y and  # Index finger is up
                               middle_tip.y > middle_pip.y and  # Middle finger is down
                               ring_tip.y > ring_pip.y and  # Ring finger is down
                               pinky_tip.y > pinky_pip.y and  # Pinky finger is down
                               thumb_tip.y > thumb_ip.y)  # Thumb is down

                if is_pointing:
                    # Check if pointing left or right relative to the wrist (INDEX_FINGER_MCP)
                    # Note: x-coordinates increase from left to right on the screen (after flipping)
                    if index_tip.x < index_mcp.x:  # Index tip is to the left of the base of the index finger
                        current_commands[handedness_label] = "POINT_LEFT"
                    else:
                        current_commands[handedness_label] = "POINT_RIGHT"
                ...
                if is_fist:
                    ...
                    cv2.putText(image, f"{handedness_label}: {current_commands[handedness_label]}",
                                (50, 50 + hand_idx * 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 0), 2)
    ...
    cv2.imshow('AI Ghost Hand - Controller V7', image)
