# final_controller_v8.py
# This version is simplified for Lamp (Left Hand) and Pump (Right Hand) control only.
...
# --- SETUP ---
print("Starting AI Ghost Hand Controller V8 (Lamp & Pump Only)...")
...
while cap.isOpened():
    ...
    for hand_label, cmd in current_commands.items():
        if hand_label == "Left":
            ...
            if cmd and cmd != last_command_right and (time.time() - last_command_time_right) > command_cooldown:
                if cmd == "THUMBS_UP":  # Only send this for right hand
    ...
    cv2.imshow('AI Ghost Hand - Controller V8', image)
